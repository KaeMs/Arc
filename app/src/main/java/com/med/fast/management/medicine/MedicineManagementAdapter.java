...
public class MedicineManagementAdapter extends FastBaseRecyclerAdapter implements MedicineCreateDeleteIntf, DeleteConfirmIntf {
        ...
        createBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mAwesomeValidation.validate()){
                    ...
                    medicineManagementModel.setRoute(routeString);
                    medicineManagementModel.setDose(doseString);
                    medicineManagementModel.setFrequency(frequencyString);
                    medicineManagementModel.setReason(reasonString);
                    medicineManagementModel.setStatus(statusString);
    ...
    private void reSubmitItem(int position){
        ...
        medicineManagementSubmitAPI.data.query.route = mDataset.get(position).getRoute();
        medicineManagementSubmitAPI.data.query.form = mDataset.get(position).getForm();
        medicineManagementSubmitAPI.data.query.dose = mDataset.get(position).getDose();
        medicineManagementSubmitAPI.data.query.frequency = mDataset.get(position).getFrequency();
        medicineManagementSubmitAPI.data.query.reason = mDataset.get(position).getReason();
        medicineManagementSubmitAPI.data.query.status = mDataset.get(position).getStatus();
    ...
    public void onDeleteConfirm(String deletionId) {
        for (int i = 0; i < getItemCount(); i++) {
            if (deletionId.equals("medicine" + mDataset.get(i).getId())) {
